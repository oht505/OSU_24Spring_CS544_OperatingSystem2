...
page_init(void)
{
	...
	// for (i = PGSIZE / PGSIZE; i < MPENTRY_PADDR / PGSIZE; i++) {
	// 		pages[i].pp_ref = 0;
	// 		pages[i].pp_link = page_free_list;
	// 		page_free_list = &pages[i];
	// }
	
	// [IOPHYSMEM, EXTPHYSMEM): in used
	...
	for (i = (EXTPHYSMEM / PGSIZE); i < npages; i++){
		// [EXT, ... , nextfree): in used  
		if ( i < PADDR(boot_alloc(0))/PGSIZE ){		
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;

		// [nextfree, ... , npages): free
		}else {		
			pages[i].pp_ref = 0;
			pages[i].pp_link = page_free_list;
			page_free_list = &pages[i];
...
page_alloc(int alloc_flags)
{
	...
	//cprintf("link: %p,	ref: %d\n",new_page->pp_link, new_page->pp_ref);
